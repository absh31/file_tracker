        ...
        <form action="./backend/editOfficer.php" method="POST">
            <div class="row">
                <table class="table align-middle">
                    <tbody>
                        <tr>
                            <td><input class="form-control" type="text" name="officerId" id="officer_id" value="<?php echo $officer['officer_id'] ?>" hidden required></td>
                        </tr>
                        <tr>
                            <td>Officer Name</td>
                        ...
                        <tr>
                            <td>Officer Role</td>
                            <td>
                                <select class="form-control" name="officerRole" id="officerDept">
                                    <?php
                                    $roleSql = $conn->prepare("SELECT * FROM tblrole WHERE role_active = 1");
                                    $roleSql->execute();
                                    while ($roleArray = $roleSql->fetch(PDO::FETCH_ASSOC)) {
                                    ?>
                                        <option class="form-control" value="<?= $roleArray['role_id'] ?>" <?php echo $roleArray['role_id'] == $officer['officer_role_id'] ? "selected" : "" ?>> <?= $roleArray['role_name'] ?></option>
                                    <?php
                                    }
                                    ?>
                                </select>
                        ...
                        <tr>
                            ...
                            <td>
                                <select class="form-control" name="officerDept" id="officerDept">
                                    ...
                                    ?>
                                        <option class="form-control" value="<?= $deptArray['dept_id'] ?>" <?php echo $deptArray['dept_id'] == $officer['officer_dept_id'] ? "selected" : "" ?>> <?= $deptArray['dept_name'] ?></option>
                                    <?php
                                    }
                                    ?>
                                </select>
                        ...
                        <tr>
                            <td>Remarks</td>
                            <td><textarea class="form-control" type="text" name="officerRemarks" id="dept_others"><?php echo $officer['officer_remarks'] ?></textarea></td>
                        </tr>
                    </tbody>
                </table>
                <div class="col text-center">
                    <a href="./officer.php" class="btn btn-danger px-4 mx-4">Cancel</a>
                    <input class="btn btn-dark px-4" type="submit" name="editOfficer" value="Save Changes">
                </div>
            </div>
        </form>
    </div>
